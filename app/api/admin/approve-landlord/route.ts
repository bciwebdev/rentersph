...
export async function POST(request: NextRequest) {
  try {
    ...
    // Determine target email (from request body or database record)
    const targetEmail = email || data?.email;

    // 2. Send Welcome Email via Resend if email exists
    if (targetEmail) {
      ...
      const emailHtmlBody = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e0e0e0; border-radius: 8px; color: #333333; line-height: 1.6;">
          ...
          <p style="font-size: 18px; font-weight: bold; color: #0070f3; margin-top: 10px;">Welcome to RentersPH!</p>
      ...
      await resend.emails.send({
        from: 'notifications@rentersph.com', // Change to your verified domain email
        to: targetEmail,
